...
class URLSessionExplicitDelegateInventoryTests: XCTestCase {
  ...
  final class OwnedSessionDelegate: NSObject, URLSessionDataDelegate {
  ...
  static var uninventoriedDelegateClass: AnyClass!
  ...
  func testAsyncDataTaskWithErrorStatusEndsExactlyOneSpan() async throws {
    ...
    XCTAssertEqual(Self.exporter.clientSpans.first?.attributes[SemanticAttributes.httpStatusCode.rawValue],
                   .int(403))
  }
  ...
  func testAsyncDataTaskToAnUnrelatedHostEndsExactlyOneSpan() async throws {
    ...
    XCTAssertEqual(Self.exporter.clientSpans.first?.attributes[SemanticAttributes.netPeerName.rawValue],
                   .string("127.0.0.1"))
  }
  ...
  /// session already has a delegate, so the request itself is unaffected. Its span is not ended,
  /// because the delegate's class is not in this suite's inventory — that is the documented cost
  /// of an explicit inventory and the reason to name your own delegate classes in it.
  func testAsyncRequestOnADelegateOwningSessionIsUnaffected() async throws {
    ...
    XCTAssertEqual((response as? HTTPURLResponse)?.statusCode, 200)
    XCTAssertEqual(Self.exporter.clientSpans.count, 0,
                   "an uninventoried session delegate cannot end the span — name it in the inventory")
  }
